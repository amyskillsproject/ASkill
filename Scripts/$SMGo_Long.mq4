...
extern int     MagicNumber=8088;
extern string  TradeComment = "";

int Width = 1376;
int Height = 768;
double takeprofit,StopLoss,StopLossPips, Slip;
int Divisor=1, ticket=0, buyOrders, totalOrders;
...
//----
  ...
  if(StringFind(Symbol(), "JPY",0) > 0)
   ...
   buyOrders = CountOrders(Symbol(), MagicNumber, OP_BUY);
         
   if(TpAsPips==0 && TpAsPrice==0) takeprofit=0;
   if (!TpAsPips==0) takeprofit= Ask + TpAsPips/FiveDigitM ;
   if (!TpAsPrice==0) takeprofit=TpAsPrice;
   
   if (SlAsPips==0 && SlAsPrice) StopLoss=0;
   if (!SlAsPips==0) StopLoss = Ask - SlAsPips/FiveDigitM;  
   if (!SlAsPrice==0) StopLoss=SlAsPrice;
   
   StopLossPips = (Ask - StopLoss) / point;
   
   if(UseExistingStops && buyOrders>0)
   {
      FirstOrderTicket=GetFirstTicketBymagic(MagicNumber); 
      StopLossPips = (Ask - FirstOrderStopLoss) / point;
   }
...
{  
   ...
   {
      ticket=OrderSend(Symbol(),OP_BUY,Lots,Ask,Slip,0,0,TradeComment,MagicNumber,0,Green);
